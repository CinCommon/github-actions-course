      ...
      <b-row>
        ...
        <b-col xl="3" md="6" @click.prevent="showInputModal(1)">
          <stats-card title="标点"
                      ...
                      sub-title="places数组/经纬度/{lat,lng}"
        ...
        <b-col xl="3" md="6">
          <stats-card title="地图选点"
                      type="gradient-info"
                      sub-title="点击城市/点击地图 -> 复制到剪切板"
                      icon="ni ni-chart-bar-32"
                      class="mb-4">
            <template slot="footer">
                <b-badge
                  class="mx-1"
                  v-for="(value, name) in placePositions"
                         :key="name"
                         href="#"
                         variant="info"
                         @click="copyPositionToClipboard(value)"
                >{{ name }}
                </b-badge>
            </template>
          </stats-card>
...
export default {
  ...
  data() {
    return {
      ...
      clickCopyToggle: false,
      placePositions: PLACE_POSITION,
      cm: undefined,
      codeValue: '',
      mapIsReady: false,
      map: undefined,
  ...
  methods: {
    ...
    drawLine() {
      ...
      console.log(routeArrays.map(route => `${route.lat},${route.lng}`).join("|"));
    ...
    copyPositionToClipboard({lat, lng}) {
      console.log('copyPositionToClipboard', lat, lng);
    },
    onMapReady({TMap, map}) {
